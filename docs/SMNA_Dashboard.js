...
#bpath = 'http://ftp1.cptec.inpe.br/pesquisa/das/carlos.bastarz/SMNA/dashboard'
...
dfs = pd.concat([df_dtc, df_bamh_T0, df_bamh_T4, df_bamh_GT4AT2], axis=1, keys=([df_dtc.name, df_bamh_T0.name, df_bamh_T4.name, df_bamh_GT4AT2.name]))
...
synoptic_time_list = ['00Z', '06Z', '12Z', '18Z']
...
synoptic_time = pn.widgets.CheckBoxGroup(name='Horário', value=[synoptic_time_list[0]], options=synoptic_time_list, inline=True)
...
def plot(variable, attribute, experiment):
    for count, i in enumerate(experiment):
        if count == 0:
            ...
            ax = df.loc[df['Observation Type'] == variable][attribute].hvplot.line(label=str(i), height=600)  
        else:
            sdf = globals()[i]
            df = dfs.xs(sdf.name, axis=1)
            ax *= df.loc[df['Observation Type'] == variable][attribute].hvplot.line(label=str(i), height=600)  
...
settings = pn.Row(pn.WidgetBox(variable, attribute, synoptic_time, pn.Column(experiment, height=250)))
pn.Column(
    '### SMNA Dashboard', 
    settings,
    plot,
